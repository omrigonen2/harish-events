...
async function renderRecipientMessage(campaign, recipient) {
  ...
  let ticketUrl = '';
  if (registration && event.ticketsEnabled && campaign.messageText.includes('{{ticketUrl')) {
    await assignTicketToken(registration);
    const baseUrl = (campaign.publicBaseUrl || process.env.PUBLIC_BASE_URL || process.env.RENDER_EXTERNAL_URL || '').trim();
    ticketUrl = baseUrl ? buildTicketUrlFromBase(baseUrl, registration.ticketToken) : '';
  }

  return replacePlaceholders(campaign.messageText, {
    parentFirstName: recipient.parentFirstName || '',
    parentLastName: recipient.parentLastName || '',
...
async function createCampaign({ event, accountId, userId, messageText, imageKey = '', name = '', publicBaseUrl = '' }) {
  ...
  const campaign = await MessageCampaign.create({
    ...
    startedAt: recipients.length ? new Date() : null,
...
async function resumeCampaign(campaignId) {
  const campaign = await MessageCampaign.findOneAndUpdate(
    { _id: campaignId, status: { $in: ['queued', 'paused'] } },
