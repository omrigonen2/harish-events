...
function normalizeStoredFields(doc, { hasChildren = true } = {}) {
  ...
  for (const f of sortByOrder(raw)) {
    const t = f.type;
    if (!allowChildren && t === 'children') continue;
...
function sanitizeFieldsFromBuilder(arr, { hasChildren = true } = {}) {
  ...
  const filtered = arr.filter((f) => allowChildren || f.type !== 'children');
  const mapped = ensureUniqueCustomFieldIds(filtered.map((f, i) => normalizeFieldShape({ ...f, order: i }, i)));
  const seen = new Set();
  const out = [];
  for (const f of mapped) {
    if (!allowChildren && f.type === 'children') continue;
